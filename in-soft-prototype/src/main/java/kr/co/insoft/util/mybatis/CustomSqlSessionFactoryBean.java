package kr.co.insoft.util.mybatis;
...
import kr.co.insoft.util.scanner.PackageClassScanner;
...
/**
 * typeAliasesPackage 수정
 * 	- 설정된 베이스 페키지를 기반으로 스캔하여 등록함
 * 
 * 
...
public class CustomSqlSessionFactoryBean extends SqlSessionFactoryBean {
	...
	protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
		...
		if (hasLength(this.typeAliasesPackage)) {
			...
			scanner.setPackageName(this.typeAliasesPackage);
			this.logger
					.debug("################################################################");
			try {
				for (BeanDefinition bean : scanner.getClasses()) {
					...
					if ( !clz.isInterface() ) {
						configuration.getTypeAliasRegistry().registerAlias(clz.getSimpleName(), bean.getBeanClassName());
						this.logger.debug("##### Scanned Package Class:: "
								+ bean.getBeanClassName() + " has been named as the " + clz.getSimpleName());						
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
