...
public class PezSqlSessionFactoryBean extends SqlSessionFactoryBean {
	private static final Logger log = LoggerFactory
			.getLogger(PezSqlSessionFactoryBean.class);
	...
	protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
		...
		/*if (!ObjectUtils.isEmpty(this.test)) {
			...
			for (Resource typeAliases : this.test) {
				if (typeAliases instanceof ClassPathResource) {
					typeAliasPackage = ((ClassPathResource) typeAliases)
							.getPath();
				} else {
					typeAliasPackage = typeAliases.toString();
				}
				configuration.getTypeAliasRegistry().registerAliases(typeAliasPackage);
				log.debug("Test package: '" + typeAliasPackage
						+ "' for aliases");
			}
			log.debug("######################################################");
		}*/
